$(function(){
	...
	/* 아이디 중복체크 */
	$("#btn_check_userId").on("click", function(){
	
		var user_id = $("#user_id");
		var idMsg = $("#id_availability");
		if(user_id.val() == "" || user_id.val() == null) {

			$("#form_idCheck").css("display", "");
			idMsgTag(user_id, idMsg, "ERROR");
		...
		// userIdCheck : 영문소문자, 숫자 입력 가능. 4~16자리
		var userIdCheck = RegExp(/^[a-z0-9]{4,16}$/);
		// 아이디 정규식 체크
        if(!userIdCheck.test(user_id.val())){
			...
			idMsg.html("아이디는 영문소문자, 숫자로 입력하세요. (4~16자)");
		...
		$.ajax({
			...
			success: function(data){
				if(data == 'SUCCESS'){
					// 사용 가능한 아이디
					$("#form_idCheck").css("display", "");
					idMsgTag(user_id, idMsg, "SUCCESS");
	...
	$("#btn_submit").on("click", function(){
		...
		} else if(user_nm.val() == null || user_nm.val() == ""){
			alert("이름을 입력해주세요.");
			user_nm.focus();
        ...
        } else if(!nameCheck.test(user_nm.val())){
			alert("이름을 올바르게 입력했는지 확인해주세요.");
			user_pw.focus();
