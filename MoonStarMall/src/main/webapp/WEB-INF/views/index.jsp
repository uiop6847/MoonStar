						...
						<div class="col-lg-3 col-md-6 mb-3">
							<div class="card h-100">
								<a href="/product/detail?pro_num=${newList.pro_num}">
									<img class="card-img-top" src="/product/displayFile?fileName=${newList.pro_main_img}">
								</a>
								<div class="card-body">
									<a href="/product/detail?pro_num=${newList.pro_num}" style="color: black; font-size: 16px;">
									${newList.pro_nm}</a>
									<c:if test="${newList.pro_discount == 0}">
										<br><span><fmt:formatNumber value="${newList.pro_price}" /></span>
									</c:if>
									<c:if test="${newList.pro_discount != 0}">
										<br><span style="text-decoration:line-through;"><fmt:formatNumber value="${newList.pro_price}" /></span>
										
										<br>
										<span style="color: #ECA4A6;">할인판매가 : 
											<fmt:formatNumber value="${newList.discount_price}" pattern="###,###,###" />원
										</span>
									</c:if>
								</div>
								<div class="card-footer" style="height: 72px;">
									<small>후기 [${newList.rew_cnt}]</small>
									<c:set var="score"><fmt:formatNumber value="${newList.avg_score}" pattern="0.0"/></c:set>
